...
### Required for "edit appointment status/notes" (B-edit-c in current plan)
- `PATCH /appointments/{id}` — to change status / update notes. Backend currently only supports book + cancel.
- **Repo:** `Database_Simulation`
- **Blocks:** the third sub-feature of the appointment edit popup. The first two (cancel, reschedule) can ship without it.
